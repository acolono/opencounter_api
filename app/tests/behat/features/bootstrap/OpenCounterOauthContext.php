...
//    $this->oauthContext = $environment->getContext('RstGroup\Behat\OAuth2\Context\OAuth2Context');
  ...
  public function iCreateOauthRequest() {
    $this->iSetHeaderWithValue('Content-Type', 'application/x-www-form-urlencoded');
  ...
  public function theResponseIsOauthFormat() {
    ...
    foreach ($expectedHeaders as $name => $value) {
      ...
      if ($given != $value) {
        ...
        throw new \Exception(sprintf("Header %s should be %s, %s given", $name, $value, $given));
      }
    }
  }
   ...
   */
  public function theResponseHasAPropertyAndItsTypeIsNumeric($propertyName, $typeString) {
    $value = $this->theResponseHasAProperty($propertyName);
  ...
  public function thatIHaveAnRefreshToken() {
    ...
    $this->response = $this->iSendARequestWithBody('POST', $url, $body_as_string);

//      $response = $this->getPostResponseFromUrl($url, $parameters);
    $data = json_decode($this->response->getBody(TRUE), TRUE);

    if (!isset($data['refresh_token'])) {
      throw new \Exception(sprintf("Error refresh token. Response: %s", $this->response->getBody(TRUE)));
    }
